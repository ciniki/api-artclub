...
function ciniki_artclub_memberImageUpdate(&$ciniki) {
	...
	//
	$strsql = "SELECT uuid, image_id FROM ciniki_artclub_member_images "
	...
	if( !isset($rc['item']) ) {
		return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'939', 'msg'=>'Contact image not found'));
	}
	$item = $rc['item'];
	...
	//
	$strsql = "UPDATE ciniki_artclub_member_images SET last_updated = UTC_TIMESTAMP()";
	...
	foreach($changelog_fields as $field) {
		if( isset($args[$field]) && $args[$field] != '' ) {
			...
			$rc = ciniki_core_dbAddModuleHistory($ciniki, 'ciniki.artclub', 
				...
				2, 'ciniki_artclub_member_images', $args['member_image_id'], $field, $args[$field]);
		}
	}
	...
	if( !isset($rc['num_affected_rows']) || $rc['num_affected_rows'] != 1 ) {
		ciniki_core_dbTransactionRollback($ciniki, 'ciniki.artclub');
		return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'940', 'msg'=>'Unable to update member image'));	
	}
	...
	if( isset($args['image_id']) && $item['image_id'] != $args['image_id']) {
		...
		$rc = ciniki_images_refClear($ciniki, $args['business_id'], array(
			...
			'object_id'=>$args['member_image_id']));
		if( $rc['stat'] == 'fail' ) {
			ciniki_core_dbTransactionRollback($ciniki, 'ciniki.artclub');
			return $rc;
		}
	...
	$ciniki['syncqueue'][] = array('push'=>'ciniki.artclub.member_image', 
		'args'=>array('id'=>$args['member_image_id']));
